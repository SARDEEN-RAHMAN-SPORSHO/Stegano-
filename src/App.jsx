...
const TextSteganography = () => {
  ...
  const encryptAdvanced = (text, password) => {
    ...
    for (let i = 0; i < layer1.length; i += blockSize) {
      ...
      for (let j = 0; j < block.length; j++) {
        const prevChar = j > 0 ? block.charCodeAt(j - 1) : block.charCodeAt(block.length - 1);
  ...
  const decryptAdvanced = (encryptedBase64, password) => {
    ...
    try {
      ...
      // Derive key from password (same process as encryption)
      ...
      for (let i = 0; i < encryptedText.length; i += blockSize) {
        ...
        for (let j = 0; j < block.length; j++) {
          ...
          // Reverse MixColumns
          const original = (
            (currentChar - prevChar * 3 - nextChar - keyChar + 65536 * 3) * 32769
          ) % 65536;
  ...
  const encryptExtreme = (text, password) => {
    ...
    for (let i = 0; i < layer1.length; i++) {
      ...
      const transformed = (
        (char + posFactor + keyFactor + timeFactor + quantumFactor) * 5 + 23
      ) % 65536;
    ...
    for (let i = 0; i < layer2.length; i += quantumBlockSize) {
      ...
      for (let j = 0; j < block.length; j++) {
        ...
        const entangled = (
          (current * 3 + left * 7 + right * 11 + quantumKey * 13) % 65536
        );
    ...
    const payload = {
      ...
      quantumHash: Array.from(derivedKey).reduce((acc, char, idx) => 
  ...
  const decryptExtreme = (encryptedBase64, password) => {
    ...
    try {
      ...
      const { data: encryptedData, salt, checksum, quantumHash, iterations } = payload;
      ...
      // Verify quantum key hash
      const calculatedHash = Array.from(derivedKey).reduce((acc, char, idx) => 
        acc + char.charCodeAt(0) * (idx + 2), 0) % 1000000;
      ...
      for (let i = 0; i < encryptedData.length; i++) {
        ...
        const decrypted = (
          char ^ keyChar1 ^ keyChar2 ^ keyChar3 ^ keyChar4 ^ 
          ((i * 23) % 256) ^ (password.length * 29) ^ (payload.timestamp % 256)
        );
      ...
      for (let i = 0; i < layer3.length; i += quantumBlockSize) {
        ...
        for (let j = 0; j < block.length; j++) {
          ...
          const right = block.charCodeAt((j + 1) % block.length);
          const current = block.charCodeAt(j);
          const quantumKey = derivedKey.charCodeAt((i + j + payload.timestamp) % derivedKey.length);
          
          // Reverse quantum entanglement
          const original = (
            (current - left * 7 - right * 11 - quantumKey * 13 + 65536 * 5) * 43691
          ) % 65536;
          originalBlock += String.fromCharCode(original);
        }
      ...
      for (let i = 0; i < layer2.length; i++) {
        ...
        const untransformed = (
          ((char - 23 + 65536) * 52429) % 65536 // Modular inverse of 5 mod 65536
          ...
          <div style={{...styles.card, textAlign: 'center'}}>
            ...
            <div style={{marginTop: '2rem', textAlign: 'center'}}>
              <p style={{color: '#64748b', fontSize: '0.75rem', letterSpacing: '1px'}}>
                SECURE SYSTEM • v4.0 • QUANTUM ENCRYPTION
              </p>
            </div>
          </div>
        </div>
      ...
      <div style={styles.mainContainer}>
          ...
          <div style={{position: 'relative'}}>
                ...
                <div style={{textAlign: 'center', color: '#60a5fa'}}>
                  ...
                  <p style={{fontWeight: '600'}}>
                    {encryptionLevel === 'basic' ? 'DECRYPTING MESSAGE' :
                     encryptionLevel === 'advanced' ? 'APPLYING AES-256 DECRYPTION' : 'APPLYING QUANTUM DECRYPTION'}
                  </p>
                  <p style={{fontSize: '0.875rem', color: '#94a3b8', marginTop: '0.5rem'}}>
                    {encryptionLevel === 'extreme' ? 'Quantum integrity verification...' : 
                     encryptionLevel === 'advanced' ? 'AES-256 block processing...' : 'Processing hidden data...'}
                  </p>
                </div>
              </div>
            )}
            ...
            >
              ...
              {encryptionLevel === 'basic' ? 'DECODE MESSAGE' :
               encryptionLevel === 'advanced' ? 'AES-256 DECODE' : 'QUANTUM DECODE'}
            </button>
        ...
        <div style={{marginTop: '3rem', textAlign: 'center', padding: '2rem'}}>
          <p style={{color: '#64748b', fontSize: '0.75rem', letterSpacing: '1px', fontFamily: 'inherit'}}>
            {encryptionLevel === 'basic' ? '🔒 BASIC ENCRYPTION • ⚡ FAST PROCESSING • v4.0' :
             encryptionLevel === 'advanced' ? '🔒 AES-256 ENCRYPTION • 🛡️ 3-LAYER SECURITY • v4.0' :
             '⚛️ QUANTUM ENCRYPTION • 🛡️ 5-LAYER MAXIMUM SECURITY • v4.0'}
          </p>
        </div>
      </div>
