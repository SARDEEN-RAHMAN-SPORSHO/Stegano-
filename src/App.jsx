...
const TextSteganography = () => {
  ...
  // Military-grade key derivation function
  const deriveKey = (password, salt, iterations = 100000) => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      for (let j = 0; j < key.length; j++) {
        ...
        const iterMod = iterations % 256;
        
        // Complex transformation mixing multiple operations
        const transformed = (
          (charCode ^ saltChar) +
          (j * 7) +
          (iterMod ^ charCode) +
  ...
  const encrypt = (text, password) => {
    ...
    const salt = timestamp + ':' + randomSalt;
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const encrypted = textChar ^ keyChar ^ (i % 256) ^ derivedKey.length;
    ...
    for (let i = 0; i < layer1.length; i++) {
      ...
      const positionFactor = (i * 13 + 7) % 256;
      const keyFactor = derivedKey.charCodeAt(i % derivedKey.length);
      const transformed = (char + positionFactor + keyFactor) % 65536;
      layer2 += String.fromCharCode(transformed);
    }
    
    // Layer 3: Reverse and additional XOR
    let layer3 = '';
    const reversed = layer2.split('').reverse().join('');
    for (let i = 0; i < reversed.length; i++) {
      const char = reversed.charCodeAt(i);
      const keyChar = derivedKey.charCodeAt((i + 5) % derivedKey.length);
      const encrypted = char ^ keyChar ^ ((i * 17) % 256);
    ...
    const payload = {
      ...
      salt: salt,
      version: '2.0',
      checksum: Array.from(text).reduce((acc, char) => acc + char.charCodeAt(0), 0) % 10000
    };
  ...
  const decrypt = (encryptedBase64, password) => {
    ...
    try {
      ...
      if (!payload.data || !payload.salt) {
        throw new Error('Invalid payload structure');
      }
      
      const { data: encryptedData, salt } = payload;
      ...
      for (let i = 0; i < encryptedData.length; i++) {
        ...
        const keyChar = derivedKey.charCodeAt((i + 5) % derivedKey.length);
        const decrypted = char ^ keyChar ^ ((i * 17) % 256);
      ...
      for (let i = 0; i < layer2.length; i++) {
        ...
        const positionFactor = (i * 13 + 7) % 256;
        const keyFactor = derivedKey.charCodeAt(i % derivedKey.length);
        const transformed = (char - positionFactor - keyFactor + 65536) % 65536;
        layer1 += String.fromCharCode(transformed);
      }
      ...
      for (let i = 0; i < layer1.length; i++) {
        ...
        const decrypted = encryptedChar ^ keyChar ^ (i % 256) ^ derivedKey.length;
        decryptedText += String.fromCharCode(decrypted);
      }
      
      // Verify checksum if available
      if (payload.checksum) {
        const calculatedChecksum = Array.from(decryptedText).reduce((acc, char) => 
          acc + char.charCodeAt(0), 0) % 10000;
        if (Math.abs(calculatedChecksum - payload.checksum) > 1) { // Allow small rounding errors
  ...
  // Convert text to zero-width characters with error detection
  const textToZeroWidth = (text) => {
    // Add error detection
    const checksum = text.split('').reduce((acc, char) => 
      acc + char.charCodeAt(0), 0).toString(16).padStart(4, '0');
    const enhancedText = checksum + '|' + text;
    
    let binary = '';
    for (let i = 0; i < enhancedText.length; i++) {
      const charCode = enhancedText.charCodeAt(i);
      // Use base-6 encoding for better efficiency
  ...
  const zeroWidthToText = (zeroWidth) => {
    ...
    const calculatedChecksum = text.split('').reduce((acc, char) => 
      acc + char.charCodeAt(0), 0).toString(16).padStart(4, '0');
  ...
  const handleEncode = async () => {
    ...
    if (encodePassword.length < 8) {
      alert('⚠️ For military-grade security, please use a password with at least 8 characters');
      return;
    }
    ...
    try {
      ...
      // Embed at random position for better stealth
      const insertPos = Math.max(10, Math.floor(Math.random() * (cover.length - 20)));
  ...
  const handleDecode = async () => {
    ...
    try {
      ...
      if (hidden.length < 10) {
        setDecodeError('⚠️ Insufficient hidden data detected');
        return;
      }
      ...
      if (!decrypted) {
        setDecodeError('❌ Failed to decode. The password is incorrect.');
        return;
      }
      ...
      if (error.message.includes('password') || error.message.includes('incorrect')) {
        setDecodeError('🔐 Incorrect password. Access denied.');
      } else if (error.message.includes('integrity') || error.message.includes('corruption')) {
        setDecodeError('🛡️ Data integrity violation. Message may be tampered.');
      } else if (error.message.includes('hidden data')) {
        setDecodeError('⚠️ No hidden data found in the provided text');
      } else {
        setDecodeError('❌ Decryption failed. Verify password and data integrity.');
      }
    } finally {
      setProcessing(false);
    }
        ...
        <div style={{...styles.mainContainer, maxWidth: '28rem'}}>
          <div style={{...styles.card, textAlign: 'center'}}>
            <div style={styles.header}>
              ...
              <h1 style={styles.title}>MILITARY-GRADE STEALTH</h1>
              <p style={styles.subtitle}>Top Secret Message Concealment System</p>
              <div style={{display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '1rem'}}>
                <span style={styles.badge}>AES-512 EQUIVALENT</span>
            ...
            <form onSubmit={handleAccessCode} style={{marginBottom: '1rem'}}>
              <div style={{marginBottom: '1.5rem'}}>
                ...
                <input
                  ...
                  placeholder="Enter top-secret clearance code..."
                  style={styles.input}
                  autoFocus
                />
              </div>
            ...
            <div style={{marginTop: '2rem', textAlign: 'center'}}>
              <p style={{color: '#64748b', fontSize: '0.75rem', letterSpacing: '1px'}}>
                TOP SECRET CLEARANCE REQUIRED • v3.0 • MILITARY GRADE ENCRYPTION
              </p>
            </div>
          </div>
        </div>
    ...
    <div style={styles.container}>
      <div style={styles.mainContainer}>
        ...
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>Top Secret Cryptographic Message Concealment</p>
          <div style={{display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '1rem', flexWrap: 'wrap'}}>
            <span style={styles.badge}>AES-512 EQUIVALENT ENCRYPTION</span>
        ...
        <div style={styles.card}>
          ...
          <div style={{color: '#94a3b8', fontSize: '0.9rem', lineHeight: '1.6'}}>
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginTop: '1rem'}}>
              <div>🔒 100,000 Iteration Key Derivation</div>
              <div>🛡️ 3-Layer Military Encryption</div>
              <div>📊 Real-time Integrity Checking</div>
              <div>🎯 Random Position Embedding</div>
              <div>🔑 Salted Password Hashing</div>
          ...
          <div style={{position: 'relative'}}>
            {processing && (
              <div style={styles.processingOverlay}>
                <div style={{textAlign: 'center', color: '#60a5fa'}}>
                  ...
                  <p style={{fontWeight: '600'}}>APPLYING MILITARY-GRADE ENCRYPTION</p>
                  <p style={{fontSize: '0.875rem', color: '#94a3b8', marginTop: '0.5rem'}}>
                    100,000 iteration key derivation in progress...
                  </p>
                </div>
              </div>
            )}
            ...
            <div style={styles.card}>
              ...
              <textarea
                ...
                placeholder="Enter top-secret message for encryption..."
                style={{
                  ...styles.textarea,
              ...
              <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.75rem'}}>
                ...
                {originalText.length > 500 && (
                  <span style={{color: '#fbbf24', fontSize: '0.875rem', fontWeight: '600', fontFamily: 'inherit'}}>
                    ⚠️ Large payload - consider splitting
                  </span>
                )}
              </div>
            </div>
            ...
            <div style={styles.card}>
              ...
              <div style={styles.inputContainer}>
                <input
                  ...
                  placeholder="Enter military-grade password (min 8 characters)..."
                  style={{
                    ...styles.input,
              ...
              <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.75rem'}}>
                <span style={{color: '#94a3b8', fontSize: '0.875rem', fontFamily: 'inherit'}}>
                  🔐 Strength: {encodePassword.length >= 12 ? 'MILITARY' : encodePassword.length >= 8 ? 'STRONG' : 'WEAK'}
                </span>
                {encodePassword.length > 0 && encodePassword.length < 8 && (
                  <span style={{color: '#f87171', fontSize: '0.875rem', fontWeight: '600', fontFamily: 'inherit'}}>
                    ❌ Minimum 8 characters required for military grade
                  </span>
                )}
              </div>
            </div>
            ...
            <button
              ...
              disabled={!originalText || !encodePassword || encodePassword.length < 8 || processing}
              style={{
                ...styles.primaryButton,
                opacity: (!originalText || !encodePassword || encodePassword.length < 8 || processing) ? 0.5 : 1,
                cursor: (!originalText || !encodePassword || encodePassword.length < 8 || processing) ? 'not-allowed' : 'pointer',
                transform: (!originalText || !encodePassword || encodePassword.length < 8 || processing) ? 'none' : 'translateY(-2px)'
              }}
            >
              <Binary size={20} />
              INITIATE MILITARY ENCRYPTION
            </button>
            ...
            {processing && (
              <div style={styles.processingOverlay}>
                <div style={{textAlign: 'center', color: '#60a5fa'}}>
                  ...
                  <p style={{fontWeight: '600'}}>DECRYPTING CLASSIFIED MESSAGE</p>
                  <p style={{fontSize: '0.875rem', color: '#94a3b8', marginTop: '0.5rem'}}>
                    Verifying integrity and applying decryption...
                  </p>
                </div>
              </div>
            )}
            ...
            <div style={styles.card}>
              ...
              <div style={styles.inputContainer}>
                <input
                  ...
                  placeholder="Enter military-grade decryption key..."
                  style={{
                    ...styles.input,
            ...
            {decodedText && (
              <div style={{...styles.card, borderColor: 'rgba(34, 197, 94, 0.4)'}}>
                ...
                <div style={styles.successCard}>
                  <p style={{color: '#86efac', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.75rem', fontFamily: 'inherit', fontWeight: '600'}}>
                    <Check size={18} />
                    ✅ MESSAGE SUCCESSFULLY DECLASSIFIED WITH MILITARY-GRADE SECURITY
                  </p>
                </div>
              </div>
            )}
        ...
        <div style={{marginTop: '3rem', textAlign: 'center', padding: '2rem'}}>
          <p style={{color: '#64748b', fontSize: '0.75rem', letterSpacing: '1px', fontFamily: 'inherit'}}>
            🔒 MILITARY-GRADE ENCRYPTION • 🛡️ 100K ITERATION KEY DERIVATION • 📡 TOP SECRET CHANNEL • v3.0
          </p>
        </div>
      </div>
